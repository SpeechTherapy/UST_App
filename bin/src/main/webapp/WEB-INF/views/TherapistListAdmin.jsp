...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
  ...
  <%@ include file="AdminHeaderCss.jsp" %> 
    <script type="text/javascript" src="/ust/resources/js/Controller/DeactivateTherapist.js"></script>
...
<h1 style="color:#00CCCC;   font-size:40px;"><b>Therapist's List</b></h1> <br><br>
  </div>
            </div>
            <!-- /.box-header -->
 ...
 float:right; margin-right:15%; background-color:transparent;  "  >
                ...
                <th><center>Name</center></th>
                  ...
                  <th><center>Status</center></th>
					...
					<tr>
						<td><a href="ViewProfileTherapist" target="_blank"onclick="sendId('<c:out  value="${item.therapistId}"/>')"><br>
						<c:out value="${item.therapistName}"></c:out></a></td>
						<td><br>
						<c:out value="${item.createdDate}"></c:out></td>				
						<c:choose>
							<c:when test="${item.user.userIsActive !='0'}">
								<td><button class="btn btn-danger"  style="width:62%;  font-size:15px;"
									onclick="sendAjaxDeactivate('<c:out  value="${item.therapistId}"/>',0)">Deactivate</button>
								</td>
							</c:when>
							<c:otherwise>
								<td><button class="btn btn-success"  style="width:62%;  font-size:15px;"
								onclick="sendAjaxDeactivate('<c:out  value="${item.therapistId}"/>',1)">Activate</button>
								</td>
							</c:otherwise>
						</c:choose>
					</tr>

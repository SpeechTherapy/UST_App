...
<div class="box box-info" style="border-top: 3px solid #333; border-left: 3px solid #333; border-right: 3px solid #333;
 border-bottom: 3px solid #333; width: 80%; margin-left:10%; margin-top:3%; margin-right:10%; ">
            ...
            <!-- /.box-header -->
            <h1 style="font-size:30px;"><center><b>Assigned Lesson</b></center></h1>
          ...
          <th style="width:20%;">Lessons Assigned</th>
                </tr>
...
<tr>
<td><a href="/ust/ViewProfilePatient" target="_blank" onclick="sendId('<c:out  value="${item.patientId}"/>')">
  <c:out value="${item.patientName}"></c:out></a> </td>
<td>  <c:out value="${item.patientAge }"/></td>
<td>   <c:out value="${item.patientGender }"/></td>
<td style="margin-left:-5%;"><c:forEach items="${item.getAssignedchapterTherapist()}" var="item1">

&nbsp;&nbsp;&nbsp;<c:out value="${item1.getLesson().getLesson()}"/> </c:forEach></td></tr>
